...
export function hasWindowsHiddenAttribute(filePath: string): boolean {
  ...
  if (!Number.isInteger(attributes) || !Number.isInteger(error)) {
    throw new TypeError("GetFileAttributesW returned an invalid result");
  }
  if (attributes !== invalidFileAttributes) {
    if (error !== 0) {
      throw new Error(`GetFileAttributesW returned attributes with error ${error}`);
    }
  ...
  throw new Error(`GetFileAttributesW failed for ${filePath} with error ${error}`);
}
